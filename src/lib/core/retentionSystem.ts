...
import { teamStore, playerStore, currentSeason, gamePhase } from '../stores/gameState';
...
export function processRetentions(teamId: string, retainedPlayerIds: string[], isMega: boolean, currentBudget: number): { success: boolean, message: string, newBudget: number } {
    let totalCost = 0;
    let players: Player[] = [];
    ...
    if (isMega && retainedPlayers.length > MEGA_AUCTION_MAX_RETENTIONS) {
        return { success: false, message: `Cannot retain more than ${MEGA_AUCTION_MAX_RETENTIONS} players in a Mega Auction.`, newBudget: currentBudget };
    }
    
    if (!isMega && retainedPlayers.length > MAX_SQUAD_SIZE) {
        return { success: false, message: `Cannot retain more than ${MAX_SQUAD_SIZE} players.`, newBudget: currentBudget };
    }
    ...
    if (totalCost > currentBudget) {
        return { success: false, message: `Not enough budget. Retention cost: $${totalCost.toLocaleString()}, Budget: $${currentBudget.toLocaleString()}`, newBudget: currentBudget };
    }
    ...
    // Apply AI retentions
    processAIRetentions(isMega);

    return { success: true, message: 'Retentions confirmed successfully.', newBudget: currentBudget - totalCost };
}

function processAIRetentions(isMega: boolean) {
    let players: Player[] = [];
    playerStore.subscribe(p => players = p)();
...
export function releaseUnretainedPlayersToPool() {
    let teams: Team[] = [];
