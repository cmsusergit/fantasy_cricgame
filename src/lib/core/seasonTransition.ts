...
import { teamStore, playerStore, currentSeason } from '../stores/gameState';
...
export function processSeasonEnd(teams: Team[], players: Player[], userTeamId: string): SeasonAwards {
  ...
  teamStore.update(currentTeams => {
    return currentTeams.map(t => {
      ...
      const netWins = wins - losses; // positive means good season, negative means bad
  ...
  // Read updated teams to populate newStars and newPlaystyle
  let finalTeams: Team[] = [];
  teamStore.subscribe(t => finalTeams = t)();

  const finalPlaystyleChanges = playstyleChanges.map(change => {
    const t = finalTeams.find(x => x.id === change.teamId);
    if (t) {
      const strength = calculateTeamStrength(t);
      return {
        ...change,
