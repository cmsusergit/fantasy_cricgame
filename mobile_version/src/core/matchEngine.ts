...
export function resolveBall(
  ...
  if (Math.random() < wicketChance) {
    ...
    if (isFreeHit && wicketType !== 'run out') {
      // Safe on Free Hit! Bowled, caught, lbw, stumped are ignored.
      // Bypassed: falls through to normal run scoring.
    } else {
      ...
      return {
        ...
        isWicket: true,
        commentary: isFreeHit 
          ? `${batter.name} is run out on a Free Hit!`
          : `${batter.name} is out! ${wicketType === 'run out' ? 'run out' : wicketType}`,
        wicketType: isFreeHit ? 'run out' : wicketType,
        fielderId
      };
    }
  }
